...
Outputs a ranked list of "most problematic" events relative to an expected pT.
...
Assumptions (matches your fitter convention):
  ...
  - TrackState fields are in the Track collection's trackStates array, associated using
    GenFitTracks.trackStates_begin/end (if available). If not available, uses flat array.

Usage examples:
  python3 scan_pt_time_by_event.py --input reco.root --expectedPt 60.88
  python3 scan_pt_time_by_event.py --input reco.root --expectedPt 60.88 --top 50 --csv out.csv
...
def scan_file(path, track_coll, expected_pt, top_n=30, event_filter=None, verbose=False):
    ...
    phi_leaf  = _find_leaf(tree, prefix + "phi")  # just to get Ndata reliably
    ...
    for iev in ev_range:
        ...
        # Most of your files seem to have 1 track anyway, but this is robust.
        per_track_pts = []
        if beg_leaf and end_leaf:
            ...
            for ti in range(n_tracks):
                ...
                if b < 0 or e < 0 or e <= b or b >= n_states:
                    continue
                e = min(e, n_states)
                # Choose a representative pT for this track.
                # Since your fitter writes one AtIP state per track often, this is usually 1 value.
                # If multiple, take median for robustness.
        ...
        # - If we have per-track pts, use the one closest to expected_pt (avoids picking some secondary junk track)
        ...
        # also record max/min in the event (to catch crazy tails)
        ...
        if verbose:
            print(f"[ev {iev:5d}] pT_primary={pt_primary:9.3f}  absErr={abs_err:9.3f}  relErr={rel_err:7.3f}  "
                  f"nStates={n_states:3d}  pTmin={pt_min:9.3f}  pTmax={pt_max:9.3f}  {assoc_note}")

    # Rank by absolute error (or rel error); abs is fine since expectedPt is constant for the file
    results.sort(key=lambda r: r["abs_err"], reverse=True)

    # Print top N
    print(f"\n[summary] scanned {len(ev_range)} event(s) (file has {n_ev} events)")
    print(f"[summary] expectedPt = {expected_pt:.6g} GeV")
    print(f"[summary] showing top {min(top_n, len(results))} worst events by |pT-expected|:\n")
    print(f"{'rank':>4s} {'event':>6s} {'pT_primary':>12s} {'absErr':>12s} {'relErr':>10s} {'nStates':>7s} {'pTmin':>12s} {'pTmax':>12s}  assoc")
    for i, r in enumerate(results[:top_n], 1):
        print(f"{i:4d} {r['event']:6d} {r['pt_primary']:12.3f} {r['abs_err']:12.3f} {r['rel_err']:10.3f} "
...
def write_csv(results, out_csv):
    with open(out_csv, "w", newline="") as fp:
        w = csv.DictWriter(fp, fieldnames=list(results[0].keys()) if results else ["event"])
        w.writeheader()
        for r in results:
            w.writerow(r)
...
def main():
    ap = argparse.ArgumentParser(description="Scan TrackState.time pT event-by-event and rank problematic events.")
    ...
    ap.add_argument("--top", type=int, default=30, help="How many worst events to print (default: 30)")
