#!/usr/bin/env python3
#DOC:
#Summary: Authoritative k4run steering for the IDEA DCH digi → GGTF tracking → GenFit2DCHFitter reco chain.
#Status: authoritative
#Usage:
#  k4run steering/runDCHTestTrackFinder.py --inputFile INPUT.root --outputFile OUTPUT.root --compactXML IDEA.xml --tgeoFile IDEA.root --modelPath model.onnx --stage fit --fitter genfit2 --gf-useMat
#  Usually called through steering/local_chain.sh for local validation or scripts/reco_job.sh for HTCondor jobs.
#Examples:
...
#Connects-To: steering/local_chain.sh, scripts/reco_job.sh, scripts/make_runtime.sh, scripts/analyze_pt_resolution_grid.py, Tracking/components/GGTF_tracking.cpp, Tracking/components/GenFit2DCHFitter.cpp
#Arguments:
#  --inputFile: input EDM4hep ROOT file from ddsim or an earlier digi/reco stage.
#  --outputFile: output EDM4hep ROOT file written by k4FWCore IOSvc.
#  --compactXML: compact DD4hep XML used by GeoSvc; required when GenFit material effects are enabled.
#  --tgeoFile: optional TGeo ROOT geometry imported by GenFit2DCHFitter as a gGeoManager fallback; should match the compact XML/material variant.
#  --dchSimHits: input DCH SimTrackerHit collection name, default DCHCollection.
#  --dchName: DD4hep DCH detector name, default DCH_v2.
#  --modelPath: GGTF ONNX model path or model spec; required for stage ggtf/fit.
#  --stage: cumulative stage selector; digi runs digitizer only, ggtf runs digi+GGTF, fit runs digi+GGTF+fitter.
#  --skipDigi: bypass digitization when the input file is already digitized.
#  --dchDigiVersion: choose DCHdigi_v01 or DCHdigi_v02; current workflow normally uses v02.
#  --xyResolution_mm, --zResolution_mm: digitizer smearing parameters in mm.
#  --dch-deadtime-ns, --dch-drift-vel-um-ns, --dch-signal-vel-mm-ns, --dch-gas-type, --rw-start-ns, --rw-duration-ns: DCHdigi_v02 timing/readout controls.
#  --tbeta, --td, --onnxChunk, --maxHitsPerEvent: GGTF clustering/model execution controls.
#  --ggtf-dropWireIfAbsDTooLarge, --ggtf-maxAbsDMM: reject unphysical wire-hit drift distances before tracking.
#  --ggtf-zeroMinSizeKeep, --ggtf-minWireFracKeep, --ggtf-promoteZeroIfGood, --ggtf-skipZeroIfSmall, --ggtf-skipZeroAlways: GGTF label-zero handling controls.
#  --ggtf-filterInputWiresByTruthPdg, --ggtf-keepTruthPdg, --ggtf-dropWireIfUnlinked, --ggtf-wireSimLinkColl: optional truth-gated debug controls; normally off for production comparisons.
#  --ggtfTracksOut: GGTF output track collection, normally CDCHTracks.
#  --fitter, --fitOut: fitter selection and output collection; current authoritative fitted output is GenFitTracks.
#  --gf-useMat, --gf-disableEloss, --gf-disableAllMat, --gf-hardDisableMatIfNoGeo: GenFit material-effects policy.
#  --gf-bz, --gf-pdg: magnetic field and particle hypothesis used by GenFit2DCHFitter.
#  --gf-sortHits, --gf-dedup, --gf-dedupTolMM, --gf-rejectNegativeLabels: input-hit hygiene controls.
#  --gf-minHitsPerTrack, --gf-minMeasurementsToFit, --gf-minFittedPointsWithFI, --gf-maxChi2Ndf: fitted-track acceptance controls.
#  --gf-useKFPreFit, --gf-kfMaxIters, --gf-useDAF, --gf-dafMaxIters, --gf-fallbackToKFIfDAFFails, --gf-tryBothMomDirs: GenFit fit-strategy controls.
#  --gf-useBiasedStateForPublish, --gf-publishStateCentralFrac, --gf-publishPTMaxGeV, --gf-invalidPTSentinel, --gf-omegaVarGood, --gf-omegaVarBad: output-state and pT-publication controls.
#  --gf-positionUnitScale, --gf-wireHalfLengthMM, --gf-maxDriftMM, --gf-maxDriftMMForHit, --gf-minDriftErrMM, --gf-maxDriftErrMM: units, wire geometry, and drift-error controls.
#  --gf-seedEndpointK, --gf-seedTangentK, --gf-seedPosSigmaMM, --gf-seedMomSigmaGeV, --gf-seedPTFallbackGeV, --gf-seedPTMinGeV, --gf-seedPTMaxGeV, --gf-seedPMinGeV, --gf-useSagittaSeed, --gf-minSagittaForSeedMM: GenFit seed construction controls.
#  --gf-minHitsForObs, --gf-obsSigmaEffMM, --gf-obsMinPhiSpanRad, --gf-obsMinChordMM, --gf-obsMinSagittaMM, --gf-obsMinScore, --gf-skipIfObsTooLow: curvature observability gates.
#  --gf-prefitOutlierVeto, --gf-outlierMaxDrop, --gf-outlierCircleResidualMM, --gf-outlierChordResidualMM, --gf-outlierMinKeep: geometry-only prefit outlier rejection controls.
#  --gf-minCovEigenvalue, --gf-statsTruncCentralFrac, --gf-diagEveryNTracks: covariance/statistics/diagnostic controls.
#  --gf-detId, --gf-useLabelAsDetId, --gf-wireAnglesAreDegrees/--gf-wireAnglesAreRadians: measurement-detector ID and wire-angle interpretation controls.
#Notes:
#  This file defines the active physics/reco algorithm sequence; operational wrappers should pass through to this file rather than duplicating algorithm configuration.
#  Material-aware GenFit comparisons require the compact XML and TGeo ROOT geometry to correspond to the same detector/material variant.
#  Version-dependent Gaudi properties are deliberately set with _set_any after construction to keep the steering robust across nearby Key4HEP/k4RecTracker builds.
#  Production CF-vs-W comparisons should keep all reco and analysis knobs fixed except the intended detector/material variant.
#Tags: authoritative, steering, k4run, dch, digitization, ggtf, genfit2, edm4hep, key4hep
#DOC_END
