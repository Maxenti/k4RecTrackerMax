#!/bin/bash
#DOC:
#Summary: Authoritative local validation wrapper for the DCH digi → GGTF → GenFit2 reco chain, with full knob forwarding and ROOT provenance stamping.
#Status: authoritative
#Usage:
#  ./local_chain.sh [INPUT] [OUTPUT] [MODEL_SPEC] [COMPACT_XML] [DCH_SIMHITS] [DCH_NAME] [TGEO_FILE]
#  STAGE=fit GF_USE_MAT=1 FITTER=genfit2 ./local_chain.sh INPUT.root OUTPUT.root model.onnx IDEA.xml DCHCollection DCH_v2 IDEA.root
#Examples:
...
#Connects-To: steering/runDCHTestTrackFinder.py, scripts/reco_job.sh, scripts/make_runtime.sh, scripts/stamp_pipeline_metadata.py, scripts/analyze_pt_resolution_grid.py
#Arguments:
#  Positional 1 INPUT: EDM4hep input ROOT file from ddsim or a previous stage.
#  Positional 2 OUTPUT: reco ROOT output path; can be local or EOS-mounted path.
#  Positional 3 MODEL_SPEC: GGTF ONNX model path/spec passed to --modelPath.
#  Positional 4 COMPACT_XML: compact DD4hep XML passed to GeoSvc; must match the studied detector/material variant.
#  Positional 5 DCH_SIMHITS: input DCH SimTrackerHit collection name, normally DCHCollection.
#  Positional 6 DCH_NAME: DD4hep DCH detector name, normally DCH_v2.
#  Positional 7 TGEO_FILE: optional TGeo ROOT geometry file passed to GenFit2DCHFitter for material-aware fitting.
#  STAGE: cumulative pipeline stage; digi, ggtf, or fit. Default fit.
#  SKIP_DIGI: set to 1 to bypass digitization when INPUT is already digitized.
#  FITTER: fitter mode passed to steering; normally genfit2.
#  FIT_OUT: fitted output collection; auto maps genfit2 to GenFitTracks and none to Tracks.
#  GGTF_LOG, FITTER_LOG: steering log levels for GGTF_tracking and GenFit2DCHFitter.
#  TIMEOUT_K4RUN: optional timeout in seconds for the k4run call; 0 disables timeout.
#  MAX_HITS: optional per-event hit cap; default 0 means no cap and is preferred for production comparisons.
#  TBETA, TD, ONNX_CHUNK: GGTF clustering/model execution controls.
#  GGTF_DROP_WIRE_IF_ABS_D_TOO_LARGE, GGTF_MAX_ABS_D_MM: reject unphysical wire-hit drift distances before GGTF.
...
#  GGTF_TRUTH_GATE, GGTF_KEEP_PDG, GGTF_DROP_UNLINKED, GGTF_WIRE_SIMLINK_COLL: optional truth-gated debug controls; normally off for production.
#  GGTF_TRACKS_OUT: GGTF output collection name, normally CDCHTracks.
#  GF_USE_MAT: enable GenFit material effects; requires a valid compact XML and matching TGeo file for robust material-aware comparisons.
#  GF_DISABLE_ELOSS, GF_DISABLE_ALL_MAT, GF_HARD_DISABLE_MAT_IF_NO_GEO: material-effects policy passed to GenFit2DCHFitter.
#  GF_BZ, GF_PDG: magnetic field and particle hypothesis used by the fitter.
#  GF_REJECT_NEGATIVE_LABELS, GF_SORT_HITS, GF_DEDUP, GF_DEDUP_TOL: input-hit hygiene controls.
#  GF_MIN_HITS_PER_TRACK, GF_MIN_MEASUREMENTS_TO_FIT, GF_MIN_FITTED_POINTS_WITH_FI, GF_MAX_CHI2_NDF: fitted-track acceptance controls.
#  GF_USE_KF_PREFIT, GF_KF_MAX_ITERS, GF_USE_DAF, GF_DAF_MAX_ITERS, GF_TRY_BOTH_MOM_DIRS, GF_FALLBACK_TO_KF_IF_DAF_FAILS: fit-strategy controls.
...
#  GF_POSITION_UNIT_SCALE, GF_WIRE_HALF_LENGTH_MM, GF_MAX_DRIFT_MM, GF_MAX_DRIFT_MM_FOR_HIT, GF_MIN_DRIFT_ERR_MM, GF_MAX_DRIFT_ERR_MM: unit conversion, synthetic wire endpoint, and drift-error controls.
#  GF_WIRE_ANGLES_DEGREES: set to 1 only if SenseWireHit wire angles are stored in degrees; default assumes radians.
#  GF_SKIP_IF_OBS_TOO_LOW, GF_MIN_HITS_FOR_OBS, GF_OBS_SIGMA_EFF_MM, GF_OBS_MIN_PHISPAN_RAD, GF_OBS_MIN_CHORD_MM, GF_OBS_MIN_SAGITTA_MM, GF_OBS_SCORE_MIN: curvature observability gate controls.
#  GF_SEED_ENDPOINT_K, GF_SEED_TANGENT_K, GF_SEED_POS_SIGMA_MM, GF_SEED_MOM_SIGMA_GEV, GF_USE_SAGITTA_SEED, GF_MIN_SAGITTA_FOR_SEED_MM, GF_SEED_PT_FALLBACK_GEV, GF_SEED_PT_MIN_GEV, GF_SEED_PT_MAX_GEV, GF_SEED_P_MIN_GEV: GenFit seed construction controls.
#  GF_PREFIT_OUTLIER_VETO, GF_OUTLIER_MAX_DROP, GF_OUTLIER_CIRCLE_RESIDUAL_MM, GF_OUTLIER_CHORD_RESIDUAL_MM, GF_OUTLIER_MIN_KEEP: geometry-only prefit outlier veto controls.
#  GF_MIN_COV_EIGENVALUE, GF_STATS_TRUNC_CENTRAL_FRAC, GF_DIAG_EVERY_N_TRACKS: covariance/statistics/diagnostic controls.
#  DCH_DIGI_VERSION, DCH_DEADTIME_NS, DCH_XY_MM, DCH_Z_MM, DCH_GAS_TYPE, DCH_DRIFT_VEL_UM_NS, DCH_SIGNAL_VEL_MM_NS, DCH_READOUT_START_NS, DCH_READOUT_DUR_NS: DCH digitizer controls.
#  STAMPER, STAMP_KEY, JOBTAG, STAMP_CONFIGS: provenance metadata stamping controls.
#Notes:
#  This is the preferred local smoke-test and debugging entry point before launching Condor reco campaigns.
#  Keep this wrapper aligned with scripts/reco_job.sh so local and batch jobs use the same physics/reco knobs.
#  For CF-vs-W or other material comparisons, use matched COMPACT_XML and TGEO_FILE paths for each variant and keep all non-geometry reco knobs fixed.
#  The script intentionally sets thread and allocator environment variables to reduce ONNX/ROOT/Gaudi memory volatility.
#  k4run.log keeps the full run log; progress.log keeps a filtered operational summary for quick checks.
#  A nonzero k4run return code may be overridden only when the output ROOT file opens and contains events; inspect k4run.log before trusting such outputs.
#  Metadata stamping is non-fatal and should not be allowed to hide reco failures.
#Tags: authoritative, local-validation, k4run, dch, digitization, ggtf, genfit2, provenance, metadata, key4hep
#DOC_END
set -euo pipefail

########## defaults  ##########
DEFAULT_INPUT="/eos/user/c/cglenn/gun_samples/1_16_2026/eta_+1.00/gun_eta+1.00_pt14.142.root"
DEFAULT_OUTPUT="/eos/user/c/cglenn/reco_samples2/Debugging/1_20_2026/CF/eta_+1.00/reco_eta+1.00_pt14.142_13_22.root"
...
: "${GF_USE_MAT:=1}"                  # 0/1  (if 1, compact XML must exist)
