...
StatusCode DCHdigi_v02::initialize() {
  ...
  if (!(m_drift_velocity_um_per_ns.value() > 0.0)) {
    ...
    info() << "Drift velocity automatically set to " << m_drift_velocity_um_per_ns.value() << " um/ns for gas type "
           << m_GasType.value() << endmsg;
  } else {
    info() << "Using user-specified drift velocity: " << m_drift_velocity_um_per_ns.value() << " um/ns" << endmsg;
  }
...
DCHdigi_v02::operator()(const edm4hep::SimTrackerHitCollection& input,
  ...
  for (const auto& [cellID, simhits] : cell_map) {
    ...
    for (const auto& hit_train : hit_train_vector) {
      if (hit_train.empty())
        continue; // Shouldn't really occur, but just to be safe

      // If the DigiHit time would fall outside the readout window, do not create a DigiHit and continue to the next
      // train
      if (hit_train.front().arrival_time_ns < m_ReadoutWindowStartTime_ns.value() ||
          hit_train.front().arrival_time_ns >
              m_ReadoutWindowStartTime_ns.value() + m_ReadoutWindowDuration_ns.value()) {
        continue;
      }
...
double DCHdigi_v02::get_default_drift_velocity_um_per_ns() const {
  ...
  case 1: // pure He
    warning()
        << "Pure Helium gas selected, but no specific drift velocity implemented. Using default value of 25 um/ns."
        << endmsg;
    return 25.0;
  case 2: // Ar(50%)-Ethane(50%)
    warning() << "Ar-Ethane gas mixture selected, but no specific drift velocity implemented. Using default value of "
                 "25 um/ns."
              << endmsg;
    return 25.0;
  case 3: // pure Ar
    warning() << "Pure Argon gas selected, but no specific drift velocity implemented. Using default value of 25 um/ns."
              << endmsg;
    return 25.0;
  default:
