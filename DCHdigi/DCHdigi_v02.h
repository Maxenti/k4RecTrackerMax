 ...
 * will create separate DigiHits
 *     - @param m_GasType Gas type for the delphes dN/dx calculation: 0: He(90%)-Isobutane(10%), 1: pure He, 2:
 * Ar(50%)-Ethane(50%), 3: pure Ar
...
private:
  ...
  Gaudi::Property<double> m_drift_velocity_um_per_ns{
      ...
      "Gas drift velocity in um/ns. If negative, automatically chosen based on GasType. Currently assumed constant for "
      "the drift time calculation."};
  ...
  Gaudi::Property<int> m_GasType{
      this, "GasType", {0}, "Gas type: 0: He(90%)-Isobutane(10%), 1: pure He, 2: Ar(50%)-Ethane(50%), 3: pure Ar."};

  // Readout window start time in ns
  Gaudi::Property<double> m_ReadoutWindowStartTime_ns{
      this, "ReadoutWindowStartTime_ns", 1.0,
      "Together with ReadoutWindowDuration_ns, defines the readout window. Any DigiHits with arrival time before "
      "ReadoutWindowStartTime_ns are discarded."};
