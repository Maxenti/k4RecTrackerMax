...
SenseWireHitCollectionData::SenseWireHitCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (s > 1e15) throw std::runtime_error("Bad data after reading: a collection is too big (extension::SenseWireHit)");
  else
    if (s == 0)
      for ([[maybe_unused]] const auto& _ : *m_data.get())
        throw std::runtime_error("Bad data after reading: zero-sized collection with data (extension::SenseWireHit)");
  // end of ugly
