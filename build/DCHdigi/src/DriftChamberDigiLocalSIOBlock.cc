...
#include "extension/DriftChamberDigiLocalSIOBlock.h"
...
void DriftChamberDigiLocalSIOBlock::read(sio::read_device& device, sio::version_type version) {
  const auto& bufferFactory = podio::CollectionBufferFactory::instance();
  // TODO:
  // - Error handling of empty optional
  auto maybeBuffers = bufferFactory.createBuffers("extension::DriftChamberDigiLocalCollection", sio::version::major_version(version), m_subsetColl);
  m_buffers = maybeBuffers.value_or(podio::CollectionReadBuffers{});

  if (not m_subsetColl) {
    unsigned size(0);
    device.data( size );
    auto* dataVec = m_buffers.dataAsVector<extension::DriftChamberDigiLocalData>();
    dataVec->resize(size);
...
void DriftChamberDigiLocalSIOBlock::write(sio::write_device& device) {
  if (not m_subsetColl) {
    auto* dataVec = podio::CollectionWriteBuffers::asVector<extension::DriftChamberDigiLocalData>(m_buffers.data);
...
namespace {
  ...
  const auto elemDriftChamberDigiLocal = extension::DriftChamberDigiLocal{};
}
