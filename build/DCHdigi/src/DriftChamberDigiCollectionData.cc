...
DriftChamberDigiCollectionData::DriftChamberDigiCollectionData() :  m_data(new DriftChamberDigiDataContainer()) {
}

DriftChamberDigiCollectionData::DriftChamberDigiCollectionData(podio::CollectionReadBuffers buffers, bool isSubsetColl) :
  ...
  if (!isSubsetColl) {
    m_data.reset(buffers.dataAsVector<extension::DriftChamberDigiData>());
  ...
  if (s > 1e15) throw std::runtime_error("Bad data after reading: a collection is too big (extension::DriftChamberDigi)");
  else
    if (s == 0)
      for ([[maybe_unused]] const auto& _ : *m_data.get())
        throw std::runtime_error("Bad data after reading: zero-sized collection with data (extension::DriftChamberDigi)");
  // end of ugly
...
void DriftChamberDigiCollectionData::clear(bool isSubsetColl) {
  if (isSubsetColl) {
...
podio::CollectionWriteBuffers DriftChamberDigiCollectionData::getCollectionBuffers(bool isSubsetColl) {
return {
...
void DriftChamberDigiCollectionData::prepareForWrite(bool isSubsetColl) {
  for (const auto& pointer : m_refCollections) { pointer->clear(); }
...
void DriftChamberDigiCollectionData::prepareAfterRead(uint32_t collectionID) {
  int index = 0;
  for (const auto& data : *m_data) {
    auto obj = new DriftChamberDigiObj({index, collectionID}, data);
...
bool DriftChamberDigiCollectionData::setReferences(const podio::ICollectionProvider* collectionProvider, bool isSubsetColl) {
  if (isSubsetColl) {
    for (const auto& id : *m_refCollections[0]) {
      podio::CollectionBase* coll = nullptr;
      extension::DriftChamberDigiObj* obj = nullptr;
      if (collectionProvider->get(id.collectionID, coll)) {
        auto* tmp_coll = static_cast<extension::DriftChamberDigiCollection*>(coll);
...
void DriftChamberDigiCollectionData::makeSubsetCollection() {
